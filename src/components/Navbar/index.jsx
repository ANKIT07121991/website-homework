  ...
  return (
    <nav
      className={`fixed w-full z-20 transition-all duration-500 ${
        scrolled ? "bg-white bg-opacity-90  shadow-lg " : "bg-gray"
      }`}
    >
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex items-center justify-between h-16 md:h-20 relative ">
          ...
          >
            <span className="text-black-500">Create</span>
            <span className="text-black-500">My</span>
            <span className="text-black-500">Website</span>
          </NavLink>

          {/* Hamburger Menu for Mobile */}
          <div
            onClick={handleOpen}
            className={`flex flex-col gap-1 cursor-pointer md:hidden transition-all duration-300 ${
          ...
          <div className="hidden md:flex items-center gap-8">
            {[
              ...
              <NavLink
                key={idx}
                to={`/${link.toLowerCase()}`}
                className="relative font-semibold text-black-500 hover:text-blue-600 transition-colors duration-200 group"
                activeClassName="text-blue-600"
                exact
              >
                {link}
                <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-black-500 transition-all duration-300 group-hover:w-full"></span>
              </NavLink>
            ))}
          </div>
          ...
          <div className="hidden md:flex items-center gap-6">
            <NavLink to="/login" className="text-black-500 hover:underline">
              Login
            </NavLink>
            <NavLink to="/start-free">
              <button className="text-white bg-black-500 hover:bg-white py-3 px-6 rounded-full transition-all duration-300">
                Start Free
              </button>
            </NavLink>
          </div>
